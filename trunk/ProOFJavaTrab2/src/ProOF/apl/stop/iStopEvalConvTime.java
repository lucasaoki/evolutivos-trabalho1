...
public class iStopEvalConvTime extends aStop {
	...
	iter = (iIteration) win.need(iIteration.class);
    }
	...
	if (_max_iter_wo_Vary) {
	    ...
	    pIterWOVary = (iter.valueWithouVary()) / max_IterWOVary;
	...
	if (_max_iter_wo_Vary && !result) {
	    result = iter.valueWithouVary() >= max_IterWOVary;
	    if (result) {
		StopResult = ("Stop by Max iteration Best Sol  without vary");
	    }
	}
	...
	if (_max_eval_wo_Vary && !result) {
	    ...
	    if (result) {
		StopResult = ("Stop by Max Eval Best Sol vary with BAND GAP");
	    }
	}
